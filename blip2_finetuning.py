...
This script replaces CLIP with BLIP-2 (Bootstrapping Language-Image Pre-training 2)
for improved vision-language understanding capabilities.

BLIP-2 Architecture:
- Q-Former: Querying Transformer that bridges vision and language modalities
- Vision Encoder: Pre-trained vision transformer (EVA-ViT)
- Language Model: Pre-trained language model (OPT/FlanT5)
...
class BLIP2EmbeddingModel:
    ...
    def encode_image(self, image_path: str) -> np.ndarray:
        """
        Generate image embedding using BLIP-2.
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Pool the features (mean pooling across sequence dimension)
    ...
    def encode_text(self, text: str) -> np.ndarray:
        """
        Generate text embedding using BLIP-2.
        ...
        try:
            ...
            with torch.no_grad():
                # Get text features through Q-Former
                outputs = self.model.get_qformer_features(**inputs)
...
class NarutoDatasetBLIP2(Dataset):
    ...
    def __init__(self, data_dir: str, split: str = 'train', processor=None):
        """
        Initialize dataset.
    ...
    def _load_data(self) -> List[Dict]:
        ...
        if os.path.exists(classes_file):
            ...
            # Check if the CSV has the expected format with one-hot encoding
            ...
            if all(col in classes_df.columns for col in possible_class_columns):
                # One-hot encoding format - convert to single class labels
                filename_to_class = {}
                for _, row in classes_df.iterrows():
                    ...
                    for class_name in possible_class_columns:
                        ...
                        # If no class has value 1, assign 'Unknown'
                        filename_to_class[filename] = 'Unknown'

            elif 'class' in classes_df.columns:
                # Traditional format with single class column
                ...
                for file in os.listdir(split_dir):
                    if file.endswith(('.jpg', '.jpeg', '.png')):
                        # Extract class from filename (assuming format: class_xxx.jpg)
                        class_name = file.split('_')[0] if '_' in file else 'unknown'
                        filename_to_class[file] = class_name
        else:
            # Infer from directory structure if no CSV available
            filename_to_class = {}
            for file in os.listdir(split_dir):
                if file.endswith(('.jpg', '.jpeg', '.png')):
                    # Extract class from filename (assuming format: class_xxx.jpg)
...
class BLIP2FineTuner:
    ...
    def __init__(self,
                 ...
                 data_dir: str = "Anime -Naruto-.v1i.multiclass",
                 output_dir: str = "results_blip2_finetuned",
                 device: str = None):
        """
    ...
    def prepare_datasets(self):
        ...
        # Load datasets
        self.datasets['train'] = NarutoDatasetBLIP2(
            self.data_dir, 'train', self.processor)
        self.datasets['valid'] = NarutoDatasetBLIP2(
            self.data_dir, 'valid', self.processor)
        self.datasets['test'] = NarutoDatasetBLIP2(
            self.data_dir, 'test', self.processor)
    ...
    def create_model(self):
        """Create BLIP-2 model with classification head."""
        logger.info("Creating BLIP-2 model with classification head...")

        # Load base BLIP-2 model for feature extraction
    ...
    def evaluate(self, split: str = 'test'):
        ...
        Args:
            split: Dataset split to evaluate on ('test', 'valid', 'train')
        """
        ...
        with torch.no_grad():
            for inputs, targets, metadata in tqdm(dataloader, desc=f"Evaluating {split}"):
                ...
                # Handle metadata correctly - it's a list of dicts in each batch
        ...
        for i, (pred, target, meta) in enumerate(zip(all_predictions, all_targets, all_metadata)):
            detailed_results.append({
                ...
                'correct': bool(pred == target),  # Convert numpy bool to Python bool
    ...
    def create_embeddings_database(self):
        """Create embeddings database for similarity search using the fine-tuned model."""
        logger.info("Creating BLIP-2 embeddings database using fine-tuned model...")

        # Load the fine-tuned model for embeddings
        best_model_path = os.path.join(self.output_dir, 'checkpoints', 'best_model.pth')
        if not os.path.exists(best_model_path):
        ...
        # Create the fine-tuned model architecture
        ...
        # Process all images in train set
        database_data = []
        ...
        # Get embedding dimension from the fine-tuned model
...
def main():
    """Main function to run BLIP-2 fine-tuning."""
